...
class RegressionBilinearRupture(unittest.TestCase, PolynomialComparison):
    ...
    def setUp(self):
        self.data = data.load_data_from_file(os.path.join(
            os.path.dirname(__file__), "bilinear.csv"))

        self.upper = pickle.load(open(os.path.join(
            os.path.dirname(__file__), "bilinear-upper.pickle"), 'rb'))
        self.lower = pickle.load(open(os.path.join(
            os.path.dirname(__file__), "bilinear-lower.pickle"), 'rb'))
    ...
    def test_upper(self):
        ...
        rupture_lower_model = ttp.LotCenteredAnalysis(self.TTP, self.order, 
                self.data)
        rupture_upper_model = ttp.LotCenteredAnalysis(self.TTP, self.order, 
                self.data)
        rupture_model = ttp.SplitAnalysis(self.yield_model, self.frac, 
                rupture_lower_model, rupture_upper_model,self.data).analyze()
...
class RegressionBilinearAllowables(unittest.TestCase):
    ...
    def setUp(self):
        ...
        rupture_upper_model = ttp.LotCenteredAnalysis(self.TTP, self.order, 
                self.data)
        self.rupture_model = ttp.SplitAnalysis(self.yield_model, self.frac, 
                rupture_lower_model, rupture_upper_model, self.data).analyze()

        rate_lower_model = ttp.UncenteredAnalysis(self.TTP, self.order, 
                self.data, time_field = 'Creep rate (%/hr)')
        rate_upper_model = ttp.UncenteredAnalysis(self.TTP, self.order, 
                self.data, time_field = 'Creep rate (%/hr)')
        self.rate_model = ttp.SplitAnalysis(self.yield_model, self.frac, 
                rate_lower_model, rate_upper_model, self.data,
    ...
    def test_regression(self):
        ...
        res = allowables.Sc_SectionII_1A_1B(self.rupture_model, self.rate_model,
                Ts)

        regression = np.array([6.56602502e+02, 5.04290857e+02, 3.87310843e+02,
            2.97466605e+02, 2.28463475e+02, 9.44485410e+01, 2.59208936e+01,
            7.11384971e+00, 1.95235775e+00, 5.35814075e-01, 1.47051288e-01])
