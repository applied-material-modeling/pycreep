...
def write_complete_report(fname, rupture_model, rate_model, yield_model,
        temperatures, times, confidence = 0.9):
    """
    ...
    res = allowables.Sc_SectionII_1A_1B(rupture_model, rate_model, temperatures, 
            full_results = True)
    ...
    for j,T in enumerate(temperatures):
        tab.cell(row=3+j, column=1, value = T)
        for i,t in enumerate(times):
            tab.cell(row=3+j, column=2+i, value = rupture_model.predict_stress(
                np.array([t]), np.array([T]), confidence = confidence)[0])
